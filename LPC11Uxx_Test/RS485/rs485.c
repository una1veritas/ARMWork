/****************************************************************************
 *   $Id:: rs485.c 4231 2010-08-05 20:07:23Z usb00423                       $
 *   Project: NXP LPC11Uxx RS485 example
 *
 *   Description:
...
void UART_IRQHandler(void)
{
  ...
  if (IIRValue == IIR_RLS)		/* Receive Line Status */
  {
	...
	if ( ((LSRValue & (LSR_PE|LSR_RDR)) == (LSR_PE|LSR_RDR)) 
						&& (LPC_USART->RS485CTRL & RS485_NMMEN) )
	{
	  Dummy = LPC_USART->RBR;
	  /* if address match, enable RX, otherwise, disable RX. */
	  if ( Dummy == LPC_USART->RS485ADRMATCH )
	  {
		LPC_USART->RS485CTRL &= ~RS485_RXDIS;	/* Enable RX */
	  }
	  else
	  {
		LPC_USART->RS485CTRL |= RS485_RXDIS;		/* Disable RX */
#if 0
		/* Depending on the revision of the silicon, this feature may or 
		maynot exist. */
		if ( (LPC_USART->FIFOLVL & 0x0F0F) != 0x0 )
		{
		  while ( LPC_USART->LSR & LSR_RDR )
		  {
			Dummy = LPC_USART->RBR;	/* Clear data from RX FIFO */
			garbageCount++;
		  }
		  return;
		}
...
void RS485Init(uint32_t baudrate)
{
  ...
  LPC_USART->LCR |= ((0x3<<4)|(0x1<<3));		/* forced "0" sticky parity, parity enable */
  LPC_USART->RS485ADRMATCH = RS485_SLAVE_ADR;	/* UART  RS485 address, 0xC0 */
#if RS485_NMM
  LPC_USART->RS485CTRL = RS485_NMMEN;		/* NMM mode */
#else
  LPC_USART->RS485CTRL = RS485_NMMEN|RS485_AADEN|RS485_RXDIS;	/* AAD mode */
#endif

  /* Below is the direction control setting. */
#if DIR_CTRL
  /* RTS can be used as direction pin out. */
#if 1
  /* UART_RTS(LPC11Uxx) as direction control. */
  LPC_IOCON->PIO0_17 &= ~0x07;    /* UART I/O config */
  LPC_IOCON->PIO0_17 |= 0x01;     /* UART RTS as direction control */
  LPC_USART->RS485CTRL |= RS485_DCTRL;
#else
  /* Multiple locations for UART DTR */
#if 1
  LPC_IOCON->PIO1_13 &= ~0x07;    /* UART I/O config */
  LPC_IOCON->PIO1_13 |= 0x01;     /* UART DTR as direction control. */ 
#endif
#if 0
  LPC_IOCON->PIO1_19 &= ~0x07;    /* UART I/O config */
  LPC_IOCON->PIO1_19 |= 0x01;     /* UART_DTR as direction control */
#endif
  LPC_USART->RS485CTRL |= (RS485_DCTRL|RS485_SEL);
#endif
#endif
...
void RS485Send(uint8_t *BufferPtr, uint32_t Length)
{
  ...
  for ( i = 0; i < Length; i++ )
  {
	...
	if ( i == 0 )
	  LPC_USART->LCR &= ~(0x1<<4);		/* Sticky parity to 1 */
	else
	  LPC_USART->LCR |= (0x1<<4);		/* Sticky parity to 0 */
	LPC_USART->LCR |= (0x1<<3);
	BufferPtr++;
  }
  return;
