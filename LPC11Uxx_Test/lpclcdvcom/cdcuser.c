...
unsigned short CDC_BulkInAvailable;
...
//  numBytesAvail = USART_available(&usart);//ser_AvailChar (&numBytesAvail);
  numBytesRead = CDC_BulkInAvailable;
  // ... add code to check for overwrite

  //numBytesRead = USART_gets(&usart, (char *)&BulkBufIn[0]); //ser_Read ((char *)&BulkBufIn[0], &numBytesAvail);
  // send over USB
  if (numBytesRead > 0) {
...
unsigned short CDC_GetSerialState (void) {
  ...
  CDC_SerialState = 0;
  temp = LPC_USART->LSR & 0x1E; //USART_linestate(&usart); //ser_LineState (&temp);
