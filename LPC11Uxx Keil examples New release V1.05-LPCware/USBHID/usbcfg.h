...
/* On the first rev. of the silicon, the VBUS debouncing occasionally show 
erratic behavior causing back voltage failure in the USB compliance test. 
It only happens when the part is SELF POWERED. When, part is bus powered, 
the S/W workaround shouldn't apply, and USB_WORKAROUND flag should be set to 
zero. The problem has been fixed in the newer version of the silicon. The
S/W left here is for backward compatibility. */
#define USB_WORKAROUND		0
