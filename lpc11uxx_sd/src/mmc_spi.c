...
)
{
	...
	Timer2 = wt;
	do {
		d = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);
...
{
	CS_HIGH();		/* CS = H */
	SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);	/* Dummy clock (force DO hi-z for multiple slave SPI) */
...
{
	CS_LOW();
	SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);	/* Dummy clock (force DO enabled) */
...
)
{
	...
	do {							/* Wait for DataStart token in timeout of 200ms */
		token = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);
		/* This loop will take a time. Insert rot_rdq() here for multitask envilonment. */
	} while ((token == 0xFF) && Timer1);
	if(token != 0xFE) return 0;		/* Function fails if invalid DataStart token or timeout */

	rcvr_spi_multi(buff, btr);		/* Store trailing data to the buffer */
	SPI_transfer(&SPI0Def, 0xff); SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF); xchg_spi(0xFF);			/* Discard CRC */
...
)
{
	...
	SPI_transfer(&SPI0Def, token); //xchg_spi(token);					/* Send token */
	if (token != 0xFD) {				/* Send data if token is other than StopTran */
		xmit_spi_multi(buff, 512);		/* Data */
		SPI_transfer(&SPI0Def, 0xff); SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF); xchg_spi(0xFF);	/* Dummy CRC */

		resp = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);				/* Receive data resp */
...
)
{
	...
	/* Send command packet */
	SPI_transfer(&SPI0Def, 0x40 | cmd); //xchg_spi(0x40 | cmd);				/* Start + command index */
	SPI_transfer(&SPI0Def, arg>>24); //xchg_spi((BYTE)(arg >> 24));		/* Argument[31..24] */
	SPI_transfer(&SPI0Def, arg>>16); //xchg_spi((BYTE)(arg >> 16));		/* Argument[23..16] */
	SPI_transfer(&SPI0Def, arg >> 8 ); //xchg_spi((BYTE)(arg >> 8));			/* Argument[15..8] */
	SPI_transfer(&SPI0Def, arg); //xchg_spi((BYTE)arg);				/* Argument[7..0] */
	n = 0x01;							/* Dummy CRC + Stop */
	if (cmd == CMD0) n = 0x95;			/* Valid CRC for CMD0(0) */
	if (cmd == CMD8) n = 0x87;			/* Valid CRC for CMD8(0x1AA) */
	SPI_transfer(&SPI0Def, n); //xchg_spi(n);

	/* Receive command resp */
	if (cmd == CMD12) SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);	/* Diacard following one byte when CMD12 */
	n = 10;								/* Wait for response (10 bytes max) */
	do
		res = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);
	while ((res & 0x80) && --n);
...
)
{
	...
	FCLK_SLOW();
	for (n = 10; n; n--) 
    SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);	/* Send 80 dummy clocks */

	ty = 0;
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI/Idle state */
		Timer1 = 1000;						/* Initialization timeout = 1 sec */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);	/* Get 32 bit return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while (Timer1 && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
				if (Timer1 && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) ocr[n] = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);
...
)
{
	...
	case GET_BLOCK_SIZE :	/* Get erase block size in unit of sector (DWORD) */
		if (CardType & CT_SD2) {	/* SDC ver 2.00 */
			if (send_cmd(ACMD13, 0) == 0) {	/* Read SD status */
				SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);
				if (rcvr_datablock(csd, 16)) {				/* Read partial block */
					for (n = 64 - 16; n; n--) SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);	/* Purge trailing data */
	...
	case MMC_GET_OCR :		/* Read OCR (4 bytes) */
		if (send_cmd(CMD58, 0) == 0) {	/* READ_OCR */
			for (n = 4; n; n--) *ptr++ = SPI_transfer(&SPI0Def, 0xff); //xchg_spi(0xFF);
			res = RES_OK;
		}
		break;
