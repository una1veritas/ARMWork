...
inline static uint8_t spiRec(void) {
	// TODO:
//  spiSend(0XFF);
//  return SPDR;
...
uint8_t Sd2Card::init(uint8_t sckRateID) {
	...
	chipSelectHigh();
	pinMode(MISO, INPUT); //SPI_MISO_PIN, INPUT);
	pinMode(MOSI, OUTPUT); //SPI_MOSI_PIN, OUTPUT);
	pinMode(SCK, OUTPUT); //SPI_SCK_PIN, OUTPUT);
