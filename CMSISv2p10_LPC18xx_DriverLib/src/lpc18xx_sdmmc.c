...
* @return 		0 on success, or error code (-1)
**********************************************************************/
static int prv_set_trans_state(void)
{
	uint32_t status;
...
* @return 		0 on success, or error code (-1)
**********************************************************************/
static int prv_set_card_params(void)
{
	uint32_t status;
	...
	int32_t end_block)
{
    ...
    if (g_card_info->card_type & CARD_TYPE_HC)
        ...
        index = start_block * g_card_info->block_len;
...
**********************************************************************/
int sdmmc_write_blocks(void* buffer,
                      int start_block,
                      int end_block)
{
    int cbWrote = (end_block - start_block + 1) *  MMC_SECTOR_SIZE;
    int status;
    int index;
