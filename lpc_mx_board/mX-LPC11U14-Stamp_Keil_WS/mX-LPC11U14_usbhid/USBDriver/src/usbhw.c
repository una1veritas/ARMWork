/****************************************************************************
 *   $Id:: usbhw.c 6904 2011-03-22 21:07:27Z usb00423                        $
...
uint32_t USB_ReadEP (uint32_t EPNum, uint8_t *pData) {
  ...
  if ( EPNum & 0x0F )
  {
	...
	accordingly too. */
	buf_flag = BufferUsed & (0x1U << EPAdr(EPNum));
	if ( buf_flag != 0 )
	{
	  addr++;		/* Use buffer 1 */
	  index++;
  ...
  *addr &= ~0x3FFFFFF;
  ...
  else
  {
	...
	BufferUsed ^= (0x1U << EPAdr(EPNum));
	BufferUsed &= LPC_USB->EPBUFCFG;
	if ( LPC_USB->EPBUFCFG & (0x1U << EPAdr(EPNum)) )
	{
