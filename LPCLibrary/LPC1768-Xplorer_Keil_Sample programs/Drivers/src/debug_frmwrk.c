...
#ifdef VCOM_DEBUG_MESSEGES
#include "usbhw.h"
#include "cdcuser.h"
#include "vcomdemo.h"
#define VCOM_DELAY 40000
#endif
#ifdef _DBGFWK
/* Debug framework */
...
#ifdef VCOM_DEBUG_MESSEGES
	int i=0;
	/* NGX: Hack!! to get the debug messages over VCOM.

	Need to figure out a better way to pass the debug
	messages to the USB VCOM port. Certainly not efficient
	way to put the debug messages out. Needs considerable
	delay to get the debug messages consistently. 

	Please disable this or switch to UART debug messages for performance intensive applications */
	for(i=0;i<VCOM_DELAY;i++);
...
void debug_frmwrk_init(void)
{
 ...
 #endif
 #ifdef VCOM_DEBUG_MESSEGES
	VCOM_Init();		//wait for usb enumeration
#endif
	_db_msg	= UARTPuts;
	_db_msg_ = UARTPuts_;
