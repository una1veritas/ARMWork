...
uint8_t i2c_write(uint8_t addr, uint8_t * data, size_t length)
{
  I2CWriteLength = length+1;
  I2CReadLength = 0;
  I2CMasterBuffer[0] = addr;
  memcpy((void*)(I2CMasterBuffer+1), data, length);
  return !I2CEngine();
}

uint8_t i2c_write16(uint8_t addr, uint16_t data) {
  uint8_t buf[2];
  buf[0] = data>>8;
  buf[1] = data&0xff;
...
uint8_t i2c_read(uint8_t addr, uint8_t * data, size_t reqlen, size_t receivelen)
{
  int i;
  
    /* Write SLA(W), address, SLA(R), and read one byte back. */
  I2CWriteLength = reqlen;
  I2CReadLength = receivelen;
  I2CMasterBuffer[0] = addr & 0xFE;
  memcpy((void*)(I2CMasterBuffer+1), data, reqlen);
