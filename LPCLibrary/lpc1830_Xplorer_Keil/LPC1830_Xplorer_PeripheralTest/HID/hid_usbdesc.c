...
uint32_t copy_descriptors(USB_CORE_DESCS_T* pDesc, uint32_t mem_base)
{
  ...
  /* set all descriptors */
  memcpy((void*)mem_base, USB_DeviceDescriptor1, sizeof(USB_DeviceDescriptor1));
  pDesc->device_desc = dest;
  dest += sizeof(USB_DeviceDescriptor1);
  ...
  memcpy(dest, USB_StringDescriptor1, sizeof(USB_StringDescriptor1));
  pDesc->string_desc = dest;
  dest += sizeof(USB_StringDescriptor1);
   /* align to 4 byte boundary */
  while ((uint32_t)dest & 0x03) dest++;

  memcpy(dest, USB_DeviceQualifier1, sizeof(USB_DeviceQualifier1));
  pDesc->device_qualifier = dest;
  dest += sizeof(USB_DeviceQualifier1);
  /* align to 4 byte boundary */
  while ((uint32_t)dest & 0x03) dest++;
