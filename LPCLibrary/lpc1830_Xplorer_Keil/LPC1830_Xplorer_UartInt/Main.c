...
void UART0_IRQHandler(void)
{
	...
	intsrc = DEBUG_UART_PORT->IIR;
	tmp = intsrc & UART_IIR_INTID_MASK;

	// Receive Line Status
	if (tmp == UART_IIR_INTID_RLS){
		// Check line status
		tmp1 = DEBUG_UART_PORT->LSR;
...
void UART_IntReceive(void)
{
	...
	while(1){
		...
		rLen = UART_Receive((LPC_USARTn_Type *)DEBUG_UART_PORT, &tmpc, 1, NONE_BLOCKING);
		// If data received
		if (rLen){
...
void UART_IntTransmit(void)
{
    ...
    UART_IntConfig((LPC_USARTn_Type *)DEBUG_UART_PORT, UART_INTCFG_THRE, DISABLE);
	...
	while (!__BUF_IS_EMPTY(rb.tx_head,rb.tx_tail))
    {
    	...
    	if (UART_Send((LPC_USARTn_Type *)DEBUG_UART_PORT, (uint8_t *)&rb.tx[rb.tx_tail], 1, NONE_BLOCKING)){
	...
	if (__BUF_IS_EMPTY(rb.tx_head, rb.tx_tail)) {
		...
		TxIntStat = SET;
    	UART_IntConfig((LPC_USARTn_Type *)DEBUG_UART_PORT, UART_INTCFG_THRE, ENABLE);
    }
}
...
void print_menu(void)
{
	...
	while(tmp) {
		tmp2 = UARTSend((LPC_USARTn_Type *)DEBUG_UART_PORT, pDat, tmp);
		pDat += tmp2;
		tmp -= tmp2;
	}
}
...
int c_entry(void)
{
	...
	UART_IntConfig((LPC_USARTn_Type *)DEBUG_UART_PORT, UART_INTCFG_RBR, ENABLE);
	/* Enable UART line status interrupt */
	UART_IntConfig((LPC_USARTn_Type *)DEBUG_UART_PORT, UART_INTCFG_RLS, ENABLE);
	/*
    ...
    while (exitflag == RESET)
    {
       len = 0;
        while (len == 0)
        {
            len = UARTReceive((LPC_USARTn_Type *)DEBUG_UART_PORT, buffer, sizeof(buffer));
        }
        ...
        while (idx < len)
        {
            if (buffer[idx] == 27)
            {
            	...
            	UARTSend((LPC_USARTn_Type *)DEBUG_UART_PORT, menu3, sizeof(menu3));
                exitflag = SET;
            }
            ...
            {
                /* Echo it back */
            	UARTSend((LPC_USARTn_Type *)DEBUG_UART_PORT, &buffer[idx], 1);
            }
            idx++;
        }
    }
    ...
    while (UART_CheckBusy((LPC_USARTn_Type *)DEBUG_UART_PORT));
