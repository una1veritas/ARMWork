...
Status I2S_FreqConfig(LPC_I2Sn_Type *I2Sx, uint32_t Freq, uint8_t TRMode) {
	...
	uint32_t i2sPclk;
	uint64_t divider;
volatile uint8_t bitrate,channel,wordwidth;
	uint32_t x, y;
	uint16_t dif;
	uint16_t error;
	...
	/* find i that make x/y <= 1 -> divider <= 2^16 */
	for(i=64;i>=0;i--){
		if((divider*i) < (1<<16)) break;
	}
	...
	x_divide = ((uint64_t)y_divide * Freq *( bitrate+1)* i * 2)/i2sPclk;
