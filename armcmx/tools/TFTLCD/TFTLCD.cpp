...
*/
#define IO8BUS_WRITE(v)  GPIOWrite(PD_ALL, (GPIORead(PD_ALL) & ~0xff) | (v & 0xff))
#define IO8BUS_READ()   GPIORead(PD_ALL)
#define IO8BUS_MODE(m)  GPIOMode(PD_ALL, 0x00ff, m, GPIO_Speed_50MHz, GPIO_OType_PP, GPIO_PuPd_UP)
...
TFTLCD::TFTLCD(const GPIOPin & cs, const GPIOPin & cd, const GPIOPin & wr, const GPIOPin & rd, const GPIOPin & rst) 
	 	 : Adafruit_GFX(TFTWIDTH, TFTHEIGHT),
	 	   _CS(cs), _WR(wr), _CD(cd), _RD(rd), _RST(rst) {
