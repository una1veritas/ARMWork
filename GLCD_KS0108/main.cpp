...
{  
  ...
  while(iter++ < 10){   // do 10 iterations
    ...
    GLCD.print((long)iter);            // print current iteration at the current cursor position 
  } 
  ...
  GLCD.print("GLCD ");
  GLCD.print((long)GLCD_VERSION, DEC);
  if(GLCD.Height <= 32)
    GLCD.CursorToXY(GLCD.CenterX + 4, 1);
  else  
    GLCD.CursorToXY(GLCD.CenterX + 4, 24);
  GLCD.print("FPS=");               // print a text string
  GLCD.print((long)fps);                  // print an integer value
  GLCD.print(".");
  if(fps_fract < 10)
    GLCD.print((long)0);             // have to manually print the leading 0 when necessary
  GLCD.print((long)fps_fract);
}
