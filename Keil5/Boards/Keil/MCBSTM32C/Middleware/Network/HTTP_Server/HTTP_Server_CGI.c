  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (strncmp (var, "ip=", 3) == 0) {
        // Local IP address
        sscanf (&var[3], "%hhu.%hhu.%hhu.%hhu", &LocM.IpAdr[0], &LocM.IpAdr[1],
                                                &LocM.IpAdr[2], &LocM.IpAdr[3]);
      }
      else if (strncmp (var, "msk=", 4) == 0) {
        // Local network mask
        sscanf (&var[4], "%hhu.%hhu.%hhu.%hhu", &LocM.NetMask[0], &LocM.NetMask[1],
                                                &LocM.NetMask[2], &LocM.NetMask[3]);
      }
      else if (strncmp (var, "gw=", 3) == 0) {
        // Default gateway IP address
        sscanf (&var[3], "%hhu.%hhu.%hhu.%hhu", &LocM.DefGW[0], &LocM.DefGW[1],
                                                &LocM.DefGW[2], &LocM.DefGW[3]);
      }
      else if (strncmp (var, "pdns=", 5) == 0) {
        // Primary DNS server IP address
        sscanf (&var[5], "%hhu.%hhu.%hhu.%hhu", &LocM.PriDNS[0], &LocM.PriDNS[1],
                                                &LocM.PriDNS[2], &LocM.PriDNS[3]);
      }
      else if (strncmp (var, "sdns=", 5) == 0) {
        // Secondary DNS server IP address
        sscanf (&var[5], "%hhu.%hhu.%hhu.%hhu", &LocM.SecDNS[0], &LocM.SecDNS[1],
