/**
  ...
  * @file    Project/STM32_CPAL_Examples/STM32_CPAL_I2C/Two_Boards_Listen_Mode/readme.txt 
...
This example shows how to use the CPAL library to control slave I2C devices
in listen mode and communicate between two different boards.

In listen mode the slave device remain waiting until it's addressed by a master 
device. The slave device proceed by read or write operation according to 
...
The example software perform the following actions:
 - At startup, Boards A & B are both in slave listen mode and wait for read or 
   write request to be received.
   
 - When key-button is pressed, for example on Board A, the master transmitter 
   mode is activated and a write request is sent to Board B followed by a 
   specific message. Once the message is correctly sent, Board A activates the 
   master transmitter mode permanently .
   
 - When Tamper-button is pressed, for example on Board A, the master receiver 
   mode is activated and a read request is sent to Board B. Once the message is 
   correctly sent from slave, Board A activates the master receiver mode permanently .
   
 - According to the push button pressed, a value is affected to a Timer which 
   generates periodic high priority interrupts. At each interrupt, a transfer is launched. 
   The period of Timer interrupts is variable depending on the push button pressed. 
...
** UserCallbacks :
   ...
   - CPAL_I2C_SLAVE_WRITE_UserCallback : This Callback is called only in Slave 
     listen mode when Write request is received from master. DMA, Interrupts and
     transfer parameters are configured in this Callback. 
            
   - CPAL_I2C_SLAVE_READ_UserCallback : This Callback is called only in Slave 
     listen mode when Read request is received from master. DMA, Interrupts and
     transfer parameters are configured in this Callback. 
...
Then these two boards should be connected as follows:
 ...
 - Connect Gnd pins of two boards.  
  ...
  - STM3210C-EVAL Set-up 
    ...
    - Make sure the Jumper JP17 "I2C_SCK" is NOT fitted on the board.
