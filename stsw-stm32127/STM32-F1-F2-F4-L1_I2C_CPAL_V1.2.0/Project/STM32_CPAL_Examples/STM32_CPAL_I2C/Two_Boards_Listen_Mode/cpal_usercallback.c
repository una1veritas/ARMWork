/**
  ******************************************************************************
  * @file    Project/STM32_CPAL_Examples/STM32_CPAL_I2C/Two_Boards_Listen_Mode/cpal_usercallback.c
...
void CPAL_I2C_TXTC_UserCallback(CPAL_InitTypeDef* pDevInitStruct)
{
  ...
  LCD_DisplayStringLine(Line5, (uint8_t*)"TRANSMIT MODE ACTIVE");
  LCD_DisplayStringLine(Line7, (uint8_t*)"   Signal Sent OK   ");
...
void CPAL_I2C_RXTC_UserCallback(CPAL_InitTypeDef* pDevInitStruct)
{
  ...
  LCD_DisplayStringLine(Line5, (uint8_t*)"RECEIVER MODE ACTIVE");
  LCD_DisplayStringLine(Line7, MEASSAGE_EMPTY);
  LCD_DisplayStringLine(Line9, MEASSAGE_EMPTY);
  ...
  // ndata =  MAX_BUFF_SIZE - sRxStructure.wNumData;
    
  /* Initialize local Reception structures */
  sRxStructure.wNumData = MAX_BUFF_SIZE;       /* Maximum Number of data to be received */
  sRxStructure.pbBuffer = tRxBuffer;        /* Common Rx buffer for all received data */
      
  /* Check the Received Buffer */
  result = Buffer_Check(tRxBuffer, (uint8_t*)tStateSignal, (uint8_t*)tSignal, (uint16_t)BufferSize);
  ...
  switch(result)
  {
    ...
    case 1: 
      ...
      LCD_DisplayStringLine(Line7, (uint8_t*)" Signal message OK  ");
      break;
...
{
  sRxStructure.wNumData = MAX_BUFF_SIZE;       
