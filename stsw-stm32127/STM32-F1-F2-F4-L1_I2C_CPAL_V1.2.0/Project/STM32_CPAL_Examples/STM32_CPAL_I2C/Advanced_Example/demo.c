...
__IO uint32_t DisplayTimeout = TIMEOUT_DEFAULT;
...
void Demo_Init(void)
{  
  /* Initialize LEDs and push-buttons mounted on EVAL board */
...
#endif /* USE_STM3210C_EVAL || USE_STM322xG_EVAL || USE_STM324xG_EVAL*/
        
  /* Configure RTC with current time. The time can be displayed on screen and
  is used for temperature reports */
  RTC_Config();
  
  /* Configures TIM7 to generate interrupt each 50ms */ 
  TIM7_Config();   
  
  /* Initialize I2C device to communicate with EEPROM using CPAL */
  ...
  /* Display Leds Control blocks on LCD */
...
{   
  ...
  IOE_FnctCmd(IOE_1_ADDR, IOE_TS_FCT, DISABLE);
...
{   
    ...
    do
    {     
      /* Disble touch screen functionality */
      IOE_FnctCmd(IOE_1_ADDR, IOE_TS_FCT, DISABLE);
...
{ 
  ...
  if (EraseMutex == 0)
  {
    ...
    *(WBuffer+2)= (uint8_t)(WReportNum & 0x00FF);
    ...
    /* Disable touch screen functionality */
    IOE_FnctCmd(IOE_1_ADDR, IOE_TS_FCT, DISABLE);
    ...
    /* If IOE Temp is equal to 0 (IOE_TempSens not ready) set default temperature (27)*/
    if (Temperature == 0)
    {
      Temperature = 300;
    }     
...
/**
  * @brief  Clear All pending IOE IT  if the interrupt line is blocked.
  * @param  None
  * @retval None
  */
void Clear_IOEPendingIT(void)
{ 
	if(!GPIO_ReadInputDataBit(IOE_IT_GPIO_PORT,IOE_IT_PIN))
  {
