...
/* Defines -------------------------------------------------------------------*/
/* I2S Codec Volume Control */
#define VOLUME_CONVERT(x)    ((x > 100)? 100:((uint8_t)((x * 255) / 100)))

/* Codecs I2C Transaction Success */
#define CODEC_I2CTRANS_SUCCESS	0
#define CODEC_I2CTRANS_FAIL		1

/* I2S ClockSettings */
/* Nemuisan said... Aassume MCLK Output Enable,16bit Width */
const uint16_t PllI2SVal[8][2] ={
/*{ N_Val, R_Val}*/ /* fs       ,error rate        ,HSE Value */
  {   344,     2},	/* 96000kHz: error rate 0.0186% @8MHz HSE */
  {   258,     3},	/* 48000kHz: error rate 0.0186% @8MHz HSE */
  {   271,     6},	/* 44100kHz: error rate 0.0183% @8MHz HSE */
  {   426,     4},	/* 32000kHz: error rate 0.0038% @8MHz HSE */
  {   429,     4},	/* 22050kHz: error rate 0.0011% @8MHz HSE */
  {   213,     4},	/* 16000kHz: error rate 0.0038% @8MHz HSE */
  {   429,     4},	/* 11025kHz: error rate 0.0011% @8MHz HSE */
  {   256,     5}	/*  8000kHz: error rate 0.0000% @8MHz HSE */ 
};
#define I2S_NVal 0
#define I2S_RVal 1
...
volatile uint32_t unDmaBufMode = INIT_RINGBUF;
volatile uint8_t  EV_I2SDMA    = IDLING;
...
uint32_t EVAL_AUDIO_Init(uint16_t OutputDevice, uint8_t Volume, uint32_t AudioFreq)
{
	...
	else
	{
		...
		DMA_ClearFlag(AUDIO_I2S_DMA_STREAM, AUDIO_I2S_DMA_FLAG_TC |AUDIO_I2S_DMA_FLAG_HT |
						AUDIO_I2S_DMA_FLAG_FE | AUDIO_I2S_DMA_FLAG_TE | AUDIO_I2S_DMA_FLAG_DME);
...
/**************************************************************************/
/*! 
  * @brief  I2S Data Transfer With DMA
  *
  * @param  pBuffer: Buffer Data to Send
  *         unSize : Size of Datas (in Bytes)
  * @retval 0
*/
/**************************************************************************/
uint32_t EVAL_AUDIO_Play(uint16_t* pBuffer, uint32_t unSize)
{
	/* Disable the I2S DMA Stream and wait for it to be disabled 1111*/
	DMA_Cmd(AUDIO_I2S_DMA_STREAM, DISABLE);
...
{
	if(DMA1->HISR & AUDIO_I2S_DMA_FLAG_HT)
	{
		EV_I2SDMA = TRANSFER_FIRST_HALF;
		unDmaBufMode = HALF_RINGBUF;
		DMA_ClearFlag(AUDIO_I2S_DMA_STREAM, AUDIO_I2S_DMA_FLAG_HT);
	}

	if(DMA1->HISR & AUDIO_I2S_DMA_FLAG_TC)
	{
		EV_I2SDMA = TRANSFER_LAST_HALF;
		unDmaBufMode = FULL_RINGBUF;
		DMA_ClearFlag(AUDIO_I2S_DMA_STREAM, AUDIO_I2S_DMA_FLAG_TC);
	}
}
...
/**************************************************************************/
/*! 
	Wait DMA Transfer or Key Inputs .
*/
/**************************************************************************/
uint8_t AUDIO_I2S_DMA_EventWait(void)
{
	/* If NO Input, Get '0' */
	EV_I2SDMA =  xgetc_n();
	if(EV_I2SDMA) return KEY_STOP;
...
*/
/**************************************************************************/
void Codec_SetI2SCLK(uint32_t samplerate)
{
	uint16_t plli2s_n,plli2s_r;
	uint8_t camp = samplerate / 1000;

	switch (camp){
		case 96:
			plli2s_n = PllI2SVal[RATE_96000kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_96000kHz][I2S_RVal];
			break;
		case 48:
			plli2s_n = PllI2SVal[RATE_48000kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_48000kHz][I2S_RVal];
			break;
		case 44:
			plli2s_n = PllI2SVal[RATE_44100kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_44100kHz][I2S_RVal];
			break;
		case 32:
			plli2s_n = PllI2SVal[RATE_32000kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_32000kHz][I2S_RVal];
			break;
		case 22:
			plli2s_n = PllI2SVal[RATE_22050kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_22050kHz][I2S_RVal];
			break;
		case 16:
			plli2s_n = PllI2SVal[RATE_16000kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_16000kHz][I2S_RVal];
			break;
		case 11:
			plli2s_n = PllI2SVal[RATE_11025kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_11025kHz][I2S_RVal];
			break;
		case 8:
			plli2s_n = PllI2SVal[RATE_8000kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_8000kHz][I2S_RVal];
			break;
		default:
			/* Anyway, Assume 44.1kHz */
			plli2s_n = PllI2SVal[RATE_44100kHz][I2S_NVal];
			plli2s_r = PllI2SVal[RATE_44100kHz][I2S_RVal];
	}

	/* Disable PLLI2S (necessary)*/
...
/**************************************************************************/
/*! 
	CIRRUS LOGIC CS43L22 Audio Codec Control Functions.
*/
/**************************************************************************/ 
...
static uint32_t Codec_Stop(uint32_t CodecPdwnMode)
{
	...
	/* Mute the output first */
	Codec_VolumeCtrl(DEFAULT_VOLMIN);
